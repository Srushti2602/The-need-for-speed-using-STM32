/*
** NOTE: SAVE AS MAIN.CPP INSIDE SRC FOLDER OF PROJECT
*/
...
int main() {
   ...
   char totalDis[12]; // Buffer to hold the string representation of total distance
    ...
    while (1) {
        ...
        char avg_vel[12]; // Buffer to hold the string representation of average velocity
        ...
        while(countSecs < 20){
            ...
            LCD.DisplayStringAt(0, LINE(4), (uint8_t *)"Distance Travelled:",CENTER_MODE);
            LCD.DisplayStringAt(0, LINE(5), (uint8_t *)finalmeasurement.c_str(),CENTER_MODE);

            LCD.DisplayStringAt(0, LINE(7), (uint8_t *)"Average Velocity:",CENTER_MODE);
            LCD.DisplayStringAt(0, LINE(8), (uint8_t *)average_velocity.c_str(),CENTER_MODE);
        ...
        sprintf(totalDis, "%d", int(totalDistance)/100); // Convert float to string
        std::string finaldistance = std::string(totalDis) + " m"; // Concatenate with units
        sprintf(dis, "%d", int(Distance)/100); // Convert float to string
        std::string finalmeasurement = std::string(dis) + " m"; // Concatenate with units
        sprintf(avg_vel, "%d", int(avg_velocity)); // Convert float to string
        ...
        LCD.DisplayStringAt(0, LINE(5), (uint8_t *)"Final Result",CENTER_MODE);

        LCD.DisplayStringAt(0, LINE(6), (uint8_t *)"Distance Travelled:",CENTER_MODE);
        LCD.DisplayStringAt(0, LINE(7), (uint8_t *)finalmeasurement.c_str(),CENTER_MODE);

        LCD.DisplayStringAt(0, LINE(9), (uint8_t *)"Average Velocity:",CENTER_MODE);
        LCD.DisplayStringAt(0, LINE(10), (uint8_t *)average_velocity.c_str(),CENTER_MODE);
        ...
        LCD.DisplayStringAt(0, LINE(15), (uint8_t *)"Total Distance:",CENTER_MODE);
        LCD.DisplayStringAt(0, LINE(16), (uint8_t *)finaldistance.c_str(),CENTER_MODE);

        LCD.DisplayStringAt(0, LINE(1), (uint8_t *)"RTES Final Project",CENTER_MODE);
        LCD.DisplayStringAt(0, LINE(2), (uint8_t *)"By",CENTER_MODE);
        LCD.DisplayStringAt(0, LINE(3), (uint8_t *)"ups2006,ad6829,og2418",CENTER_MODE);
